...
// Struct representing the response data from each host.
...
// Takes a string of comma-separated hosts and returns a ServerList.
func NewServerList(arg string) (ServerList, error) {
  return strings.Split(arg, ","), nil
}

// Convenience function for using password auth (prompt) in the
...
// Convenience function for using public key auth in the
// []ssh.AuthMethod param in a ssh.ClientConfig.
...
// Convenience function for using SSH agent auth in the
// []ssh.AuthMethod param in a ssh.ClientConfig.
